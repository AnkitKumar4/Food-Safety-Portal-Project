/*
 * Generated by MyEclipse Struts
 * Template path: templates/java/JavaClass.vtl
 */
package com.food.strutsaction;

import java.util.Vector;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import org.apache.struts.action.Action;
import org.apache.struts.action.ActionForm;
import org.apache.struts.action.ActionForward;
import org.apache.struts.action.ActionMapping;

import com.food.delegate.RegionalInfoDelegate;
import com.food.exception.ConnectionException;
import com.food.formbean.RegionalFormBean;
import com.food.pojo.State;
import com.food.util.UtilConstants;

/**
 * MyEclipse Struts Creation date: 09-17-2012
 * 
 * XDoclet definition:
 * 
 * @struts.action path="/districtViewStateAction" name="regionalFormBean"
 *                scope="request" validate="true"
 * @struts.action-forward name="failure" path="/AddDistrict.jsp"
 * @struts.action-forward name="success" path="/AddDistrict.jsp"
 */
public class DistrictViewStateAction extends Action {
	/*
	 * Generated Methods
	 */

	/**
	 * Method execute
	 * 
	 * @param mapping
	 * @param form
	 * @param request
	 * @param response
	 * @return ActionForward
	 */
	public ActionForward execute(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) {
		Vector<State> vStates = null;
		try {
			vStates = new RegionalInfoDelegate().viewStates();
			request.setAttribute("vStates", vStates);
			return mapping.findForward(UtilConstants._SUCCESS);
		} catch (ConnectionException e) {
			request.setAttribute(UtilConstants._STATUS, e.getMessage());
			return mapping.findForward(UtilConstants._FAILURE);
		} catch (Exception e) {
			request.setAttribute(UtilConstants._STATUS,
					UtilConstants._VIEW_STATE_FAILURE);
			return mapping.findForward(UtilConstants._FAILURE);
		}

	}
}